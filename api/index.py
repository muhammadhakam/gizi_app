...
Flask_App.secret_key ="gizi"

# mysql config
Flask_App.config['MYSQL_HOST'] = 'sql12.freesqldatabase.com'
Flask_App.config['MYSQL_USER'] = 'sql12594574'
Flask_App.config['MYSQL_PASSWORD'] = 'mXns4f3zdK'
Flask_App.config['MYSQL_DB'] = 'sql12594574'
...
@Flask_App.route('/database/hasil')
def hasil():
    cursor = mysql.connection.cursor()
    cursor.execute(''' SELECT * FROM antro''')
    antro = cursor.fetchall()
...
@Flask_App.route('/database/delete/<int:id>', methods=['GET'])
def deletepasien(id):
...
@Flask_App.route('/database/tambah', methods=['GET'])
def tambah():

    return render_template("index.html")


@Flask_App.route('/database/tambah/operation_result/', methods=['POST'])
